...
char *tokenStrings[] = {"TK_SQL_DELIMITER",
                        ...
                        "TK_RETURNS_KEYWORD",
                        "TK_LIST_KEYWORD",
                        "________________UNKNOWN_TOKEN_ERROR"};
...
Keywords keywords[] = {{"int", TK_INT_KEYWORD},
                       ...
                       {"and", TK_LOGICAL_AND_KEYWORD},
                       {"or", TK_LOGICAL_OR_KEYWORD},
                       {"import", TK_IMPORT_KEYWORD},
                       {"call", TK_CALL_KEYWORD},
                       {"read", TK_READ_KEYWORD},
                       {"write", TK_WRITE_KEYWORD},
                       {"return", TK_RETURN_KEYWORD},
                       {"returns", TK_RETURNS_KEYWORD},
                       {"list", TK_LIST_KEYWORD}};
...
FILE *initLexer(char *input_code_file)
{
    ...
    if (fp == NULL)
    {
        // fprintf(stderr, "File not opened, No such file ----- ERROR\n");
        return NULL;
    }
    else
    {
...
void printAllTokens(char *input_code_file)
{
    ...
    if (endoffile)
        return;
    input_code_file = "modified_input_code.txt";
    ...
    while (token != NULL)
    {
        ...
        if (!strcmp(tokenStrings[token->type], "TK_INTEGER_LITERAL") || !strcmp(tokenStrings[token->type], "TK_FLOATING_POINT_LITERAL") || !strcmp(tokenStrings[token->type], "TK_IDENTIFIER"))
            ...
            checkOperator = 0;
        // if (strcmp(tokenStrings[token->type], "UNKNOWN_TOKEN_ERROR"))
